    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col h-full">
      <div className="p-5 flex-grow">
        ...
        <div className="space-y-3 mb-6 text-sm">
          ...
          {/* Direct Call Feature */}
          {(role === UserRole.NGO || post.status === PostStatus.COLLECTED) && (
            <div className="pt-2">
              <a 
                href={`tel:${post.contactNumber}`}
                className="flex items-center text-green-700 font-bold hover:text-green-800 transition-colors group bg-green-50 px-3 py-2 rounded-lg"
              >
                ...
                {post.contactNumber}
                <span className="ml-auto text-[10px] uppercase font-bold text-green-600">Call Provider</span>
              </a>
            </div>
          )}
        </div>
